...
export class SessionMemoryManager {
  ...
  shouldExtract(messages: Message[]): boolean {
    ...
    // 检查工具调用次数阈值
    const toolCallsSinceLastUpdate = this.countToolCallsSince(
      messages,
   ...
   * 确保会话记忆文件存在
   */
  private async ensureSessionFile(): Promise<string> {
   ...
   * 加载会话记忆模板
   */
  private loadSessionTemplate(): string {
    return `---
...
`
   ...
   */
  private checkLastTurnHasToolCalls(messages: Message[]): boolean {
    const lastMessage = messages[messages.length - 1]
