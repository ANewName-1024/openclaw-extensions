 ...
 */
export async function validatePathWithSymlinks(baseDir, filePath) {
    ...
    try {
        ...
        if (!realPath.startsWith(realBase + path.sep) && realPath !== realBase) {
            throw new SymlinkEscapeError(`Path "${filePath}" escapes via symlink: resolved to "${realPath}"`);
        }
        return realPath;
    }
    catch (e) {
 ...
 */
async function resolveSymlinks(p) {
    try {
 ...
 */
async function resolveSymlinksUp(p) {
    let current = p;
 ...
 */
export async function validateFileCount(dirPath, maxFiles) {
    try {
 ...
 */
export function createSecurePathValidator(options = {}) {
    const opts = { ...DEFAULT_SECURITY_OPTIONS, ...options };
    return async function validateSecurePath(baseDir, filePath) {
        // Sanitize input
        const sanitized = sanitizePathKey(filePath);
        // Check depth
        const depth = sanitized.split('/').filter(Boolean).length;
        if (depth > opts.maxPathDepth) {
            throw new PathTraversalError(`Path depth ${depth} exceeds maximum ${opts.maxPathDepth}`);
        }
        // Check extension
        const ext = path.extname(sanitized);
        if (!opts.allowedExtensions.includes(ext)) {
            throw new PathTraversalError(`Extension "${ext}" not allowed. Allowed: ${opts.allowedExtensions.join(', ')}`);
        }
