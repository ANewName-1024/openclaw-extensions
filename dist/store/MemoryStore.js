...
import { parseFrontmatter, serializeFrontmatter, generateFilename, validateFrontmatter, } from '../utils/frontmatter.js';
import { validatePath, validatePathWithSymlinks, } from '../security/pathValidator.js';
import { normalizePagination } from '../types/index.js';
import { MEMORY_TYPES, DEFAULT_MEMORY_CONFIG, } from '../types/index.js';
// =============================================================================
// MemoryStore
// =============================================================================
export class MemoryStore {
    ...
    async save(memory) {
        ...
        if (memory.content.length > this.config.maxFileSize) {
            throw new Error(`Memory content exceeds maximum size of ${this.config.maxFileSize} bytes`);
        }
        // Serialize and save
        const now = new Date();
        const fullMemory = {
     ...
     */
    async update(type, name, updates) {
     ...
     */
    async scanPaginated(signal, pagination) {
     ...
     */
    async scanTypePaginated(type, signal, pagination) {
     ...
     */
    async searchPaginated(options, pagination) {
        let headers = await this.scan();
        // Filter by type
        if (options.type) {
            headers = headers.filter(h => h.type === options.type);
     ...
     */
    async getIndex(scope = 'private') {
        const indexPath = path.join(this.getMemoryDir('project', scope), 'MEMORY.md');
        try {
    ...
    async updateIndex(memory) {
        const indexPath = path.join(this.getMemoryDir(memory.type, memory.scope), 'MEMORY.md');
     ...
     */
    async removeFromIndex(type, name) {
        const scope = this.guessScope(type);
        const indexPath = path.join(this.getMemoryDir(type, scope), 'MEMORY.md');
        try {
    ...
    getTypeDirs(type) {
        const dirs = [path.join(this.baseDir, type)];
