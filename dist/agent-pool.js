...
export class AgentPool extends EventEmitter {
    ...
    lifecycleManager;
    statsCollector;
    constructor(agentFactory, tierConfigs) {
        super();
     ...
     * 归还 Agent 到池
     */
    async releaseAgent(agent, taskResult) {
...
export class LifecycleManager {
    ...
    start() {
        ...
        this.running = true;
        // 健康检查 (每 30 秒)
        this.intervals.push(setInterval(() => this.healthCheck(), 30000));
        // 池补充 (每 60 秒)
        this.intervals.push(setInterval(() => this.replenish(), 60000));
        // 统计收集 (每 5 分钟)
        this.intervals.push(setInterval(() => this.collectStats(), 300000));
    }
    stop() {
        this.running = false;
...
export class StatsCollector {
    ...
    collect() {
        const stats = new Map();
        for (const [tier, agents] of this.pools) {
...
export class DefaultAgentFactory {
    ...
    async executeTask(sessionKey, task) {
        // TODO: 执行任务
