...
export class MemorySelector {
    ...
    constructor(config = {}, modelClient) {
        this.config = { ...DEFAULT_MEMORY_CONFIG.aiSelection, ...config };
        this.modelClient = modelClient;
    }
    /**
     * Select the most relevant memories for a query
     */
    async select(query, memories, options = {}) {
     ...
     */
    async aiSelect(query, memories, maxResults, recentTools) {
        ...
        const selectedNames = JSON.parse(response).selected_memories || [];
     ...
     */
    keywordSelect(query, memories, maxResults) {
     ...
     */
    filterRecentToolMemories(memories, recentTools) {
...
// Factory
// =============================================================================
export function createMemorySelector(config, modelClient) {
