...
export class MemoryExporter {
     ...
     */
    async exportToDirectory(dirPath) {
        const data = await this.exportToJSON();
        // Create directory structure
        await fs.mkdir(dirPath, { recursive: true });
        // Save metadata
        await fs.writeFile(path.join(dirPath, 'metadata.json'), JSON.stringify(data.metadata, null, 2), 'utf8');
        ...
        for (const [type, memories] of Object.entries(byType)) {
            await fs.writeFile(path.join(dirPath, `${type}.json`), JSON.stringify(memories, null, 2), 'utf8');
        }
        return data.metadata;
    }
}
...
export class MemoryImporter {
     ...
     */
    async importFromData(data, options = {}, startTime = Date.now()) {
        const opts = {
            overwrite: options.overwrite ?? false,
            skipOnError: options.skipOnError ?? true,
            validateOnly: options.validateOnly ?? false,
 ...
 */
export async function createBackup(store, backupDir) {
